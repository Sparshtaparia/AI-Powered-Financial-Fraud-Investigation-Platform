...
def test_investigate_partial_failure(mock_evidence, mock_graph, mock_ml):
    # Setup mocks
    mock_ml.return_value = ServiceResult(
        service="ml",
        success=True,
        latency_ms=10,
        payload={"risk": 0.9, "label": "HIGH"},
    )
    mock_graph.return_value = ServiceResult(
        service="graph", success=False, latency_ms=10, error="Timeout"
    )
    mock_evidence.return_value = ServiceResult(
        service="evidence", success=True, latency_ms=10, payload={"merkle_root": "ABC"}
    )
